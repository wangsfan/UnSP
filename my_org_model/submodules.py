...
class UpsampleConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, activation='relu', norm=None):
        ...
        if norm == 'BN':
            # self.norm_layer = nn.BatchNorm2d(out_channels)
            self.norm_layer = SPADE(out_channels, 3)
        elif norm == 'IN':
            self.norm_layer = nn.InstanceNorm2d(out_channels, track_running_stats=True)
...
class UpConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, scale=2, stride=1, padding=0, activation='relu', norm=None):
        ...
        bias = False if norm == 'BN' else True
        self.conv0 = nn.Conv2d(self.in_plane, self.planes, kernel_size=kernel_size, padding=padding, stride=stride, bias=bias)
        ...
        # self.norm = SPADE(self.out_plane, 3)
